...
Here is your ${}.'.format(money)
    ...
    def vend(self):
        if self.inventory == 0:
            ...
            return 'You must add ${} \
more funds.'.format(self.price - self.balance)
            ...
            self.balance = 0
            return 'Here is your {} and \
${} change.'.format(self.product, change)
...
def is_bst(t):
    ...
    def LDR(node):
        ...
        if len(node.branches) == 2:
            ...
            if node.branches[0].label < node.label:
                LDR(node.branches[0])
                nodes.append(node.label)
            else:
                nodes.append(node.label)
