...
def subseqs(s):
    ...
    if not s:
        ...
        next = subseqs(s[1:])
        return next + insert_into_all(s[0], next)
...
def make_glookup(class_assignments):
    ...
    total_mark = 0
    total_grade = 0
    def student(asm, grade):
        nonlocal total_mark,total_grade
        total_mark += class_assignments[asm]
        total_grade += grade
        return total_grade/total_mark
    return student
